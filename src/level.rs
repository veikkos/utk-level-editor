    ...
    pub fn delete_crate_if_near(&mut self, level_coordinates: &Position) {
        let mut to_be_removed = Vec::new();
        {
            let distances: Vec<_> = self
                ...
                .staticc
                .iter()
                .map(|(crate_coordinates, &_crate)| {
                    let distance =
                        get_distance_between_points(level_coordinates, crate_coordinates);
                    (crate_coordinates, distance)
                })
                .collect();
            for crate_item in distances {
                // TODO: Box implementation
                if 5.0 as f64 >= crate_item.1 * RENDER_MULTIPLIER as f64 {
                    to_be_removed.push(*crate_item.0);
                }
            }
        }
        for key in to_be_removed {
